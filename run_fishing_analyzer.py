...
def analyze_mpa_proximity(fishing_data, mpa_data):
    ...
    mpa_geometries = mpa_data.get('geometries', [])
    fishing_events = fishing_data.get('entries', [])
    if not mpa_geometries or not fishing_events: return None
    ...
    for i, event in enumerate(fishing_sample):
        if i % 100 == 0: print(f"    - Analyzing event {i}/{sample_size}...", flush=True)
...
def analyze_global_hotspot(fishing_data, mpa_data):
    print("\n--- Starting Story Analysis: Global Fishing Hotspot ---", flush=True)
    fishing_events = fishing_data.get('entries', [])
    if not fishing_events: return None
...
def analyze_eez_focus(fishing_data, mpa_data):
    ...
    if not eez_counts:
        print("  - ❌ No EEZ data found in fishing events.", flush=True); return None
    ...
    for event in fishing_events:
        if event.get('regions', {}).get('eez') and event['regions']['eez'][0] == most_common_eez:
            sample_coord = [event['position']['lon'], event['position']['lat']]; break
    story_data = {
        "story_type": "eez_focus", "eez_name": most_common_eez,
        "event_count": event_count, "center_coords": sample_coord
    }
...
def generate_insight_with_ai(story_data, client):
    ...
    if story_type == 'mpa_proximity':
        distance_text = story_data['distance_text']; fishing_coords = story_data['fishing_coords']
        prompt = f"""You are a science communicator. Analyze: A fishing vessel was detected {distance_text} from an MPA boundary at {fishing_coords}. Create a JSON object with keys "tag" (#Fishing), "content" (3-4 sentences on the region and 'fishing the line' using cautious language), and "map_view" (center: {fishing_coords}, zoom: 9, maxZoom: 14). Return ONLY the raw JSON object."""
    elif story_type == 'global_hotspot':
        center_coords = story_data['center_coords']; event_count = story_data['event_count']
        prompt = f"""You are a science communicator. Analyze: The world's busiest fishing hotspot is the 5x5 degree area at {center_coords}, with {event_count} events. Create a JSON object with keys "tag" (#Fishing), "content" (3-4 sentences on the ocean basin's significance and the industrial scale), and "map_view" (center: {center_coords}, zoom: 5, maxZoom: 10). Return ONLY the raw JSON object."""
    elif story_type == 'eez_focus':
        eez_name = story_data['eez_name']; event_count = story_data['event_count']; center_coords = story_data['center_coords']
        prompt = f"""You are a science communicator. Analyze: The most active fishing zone was the EEZ of '{eez_name}', with {event_count} events. Sample at {center_coords}. Create a JSON object with keys "tag" (#Fishing), "content" (3-4 sentences on the country, the EEZ's significance, and what an EEZ is), and "map_view" (center: {center_coords}, zoom: 5, maxZoom: 10). Return ONLY the raw JSON object."""
    else:
        print("  - ❌ Error: Unknown story type.", flush=True); return None
    try:
