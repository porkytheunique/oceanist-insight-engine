...
def analyze_coral_proximity(platform_data, coral_data):
    ...
    idx = index.Index()
    for pos, coral_shape in enumerate(coral_shapes):
        idx.insert(pos, coral_shape.bounds)
    print("  - ✅ Spatial index built successfully.")
        
    # Find a random platform to analyze
    ...
    # Query the index to find the 5 nearest coral reefs
    ...
    for coral_idx in nearest_coral_indices:
        distance = platform_point.distance(coral_shapes[coral_idx])
